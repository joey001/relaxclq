...
public:
	...
	void search() ;
	static int main_maximum_kplex(char *file_path,int k_value,int lower_bound,int binary_flag,double time_limit);
...
private:
	...
	ui degen(ui n, ui *peel_sequence, ui *core, ept *pstart, ui *edges, ui *degree, char *vis, LinearHeap *heap, bool output) ;
	...
	ept peeling(ui critical_vertex, LinearHeap *linear_heap, ui *Qv, ui &Qv_n, ui d_threshold, ui *Qe, bool initialize_Qe, ui t_threshold, ui *tri_cnt, ui *active_edgelist, ui &active_edgelist_n, ui *edge_list, ui *edgelist_pointer, char *deleted, ui *degree, ept *pstart, ept *pend, ui *edges, char *exists) ;
