...
int Graph::main_maximum_kplex(char *file_path, int k_value,int lower_bound, int binary_flag, double time_limit){
	Graph *graph = new Graph(file_path, k_value);
	tl=time_limit*1000000;
	...
	if(binary_flag==1){
		if(strcmp(file_path + pos, "txt") == 0 ||strcmp(file_path + pos, "clq") == 0){
			...
			graph->dir = string(file_path);
			graph->read();
			printf("reading Time: %s (microseconds)\n",Utility::integer_to_string(readingTime.elapsed()).c_str());
			if(!setjmp(time_out))
			graph->search();         
			graph->write();
		}else{
...
void Graph::readSNAPText(const char* filepath) {
	std::ifstream infile;
	char buf[1024];
	std::vector<std::pair<ui, ui> > epairs;
	std::vector<ui> nodes;
...
void Graph::read() {
	...
	ui tt;
	utils::fread_wall(&tt, sizeof(int), 1, f);
	utils::fread_wall(&n, sizeof(int), 1, f);
	utils::fread_wall(&m, sizeof(int), 1, f);

	printf("n = %s; m = %s (undirected)\n", Utility::integer_to_string(n).c_str(), Utility::integer_to_string(m/2).c_str());

	ui *degree = new ui[n];
	utils::fread_wall(degree, sizeof(unsigned int), n, f);
	if(pstart == nullptr) pstart = new ept[n+1];
	if(edges == nullptr) edges = new ui[m];
	...
	for(ui i = 0;i < n;i ++) {
		if(degree[i] > 0) {
			utils::fread_wall(edges+pstart[i], sizeof(int), degree[i], f);
...
void Graph::write() {
	if(TimeOut){
	printf("Maximum kPlex Size(may not optimal): %lu, Total Time: %s (microseconds)\n", kplex.size(), Utility::integer_to_string(timeOfAll.elapsed()).c_str());              		
	}
	FILE *fout = Utility::open_file("output/kplexes.txt", "w");
	fprintf(fout, "%lu\n", kplex.size());
...
void Graph::search() {
	...
	if(kplex.size()>=lowerBound)
	printf("\tMaximum kPlex Size: %lu, Total Time: %s (microseconds)\n", kplex.size(), Utility::integer_to_string(timeOfAll.elapsed()).c_str());
	else
	printf("\tThere are not a k-plex not less than %u, Total Time: %s (microseconds)\n",lowerBound,Utility::integer_to_string(timeOfAll.elapsed()).c_str());
...
ui Graph::degen(ui n, ui *peel_sequence, ui *core, ept *pstart, ui *edges, ui *degree, char *vis, LinearHeap *heap, bool output) {
	Timer t;
	...
	if(new_size != 0) {
		...
		if(new_size - idx > kplex.size()) {
			...
			if(!output && new_size - idx>=lowerBound) printf("Find a k-plex of size: %u\n", new_size - idx);
		}
	}
	return UB;
}
...
ept Graph::peeling(ui critical_vertex, LinearHeap *linear_heap, ui *Qv, ui &Qv_n, ui d_threshold, ui *Qe, bool initialize_Qe, ui t_threshold, ui *tri_cnt, ui *active_edgelist, ui &active_edgelist_n, ui *edge_list, ui *edgelist_pointer, char *deleted, ui *degree, ept *pstart, ept *pend, ui *edges, char *exists) {
	ept Qe_n = 0;
