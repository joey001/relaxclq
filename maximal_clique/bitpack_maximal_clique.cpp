...
int BPMaximalClique::main_maximal_clique(char* path,int lower_bound,int binary_flag,double time_limit){
    gettimeofday(&time_start, NULL);
    BPMaximalClique* mc=new BPMaximalClique();
    int pos = fileSuffixPos(path);
    lower_b=lower_bound;
    int mc_num=0;
...
int BPMaximalClique::maximal_clique_degen()
{
    ...
    gettimeofday(&time_end, NULL);
    if(((time_end.tv_sec - time_start.tv_sec) * 1000.0 + (time_end.tv_usec - time_start.tv_usec) / 1000.0)>=time_l){
        longjmp(time_out_MC,1);
    }
    std::vector<int> R;
    R.reserve(2048);
    R.push_back(-1);
...
void BPMaximalClique::Tomita(std::vector<int>& R, UVertex P, UVertex X)
{
    gettimeofday(&time_end, NULL);
    if(((time_end.tv_sec - time_start.tv_sec) * 1000.0 + (time_end.tv_usec - time_start.tv_usec) / 1000.0)>=time_l){
        longjmp(time_out_MC,mc_num+1);
    }
    if(P.deg*PACK_WIDTH+R.size()<lower_b)
    return;
    if (P.deg == 0) {
        if (X.deg == 0) {
