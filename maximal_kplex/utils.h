...
inline FILE *open_file(const char *file_name, const char *mode) {
...
inline void fread_wall(void * __ptr, size_t __size, size_t __n, FILE* __stream){
...
inline intT interSection(intT *lst1, intT sz1, intT *lst2, intT sz2, intT* dest) {
    int i = 0, j = 0;
    int szdest = 0;
    while (i < sz1 && j < sz2) {
        if (lst1[i] < lst2[j]) ++i;
...
inline int commonEle(intT *lst1, const intT sz1, intT *lst2, const intT sz2) {
    int i = 0, j = 0;
    int szdest = 0;
    while (i < sz1 && j < sz2) {
        if (lst1[i] < lst2[j]) ++i;
