#include "../maximum_kplex/Timer.h"
#include "GraphClique.h"
#include <setjmp.h>
jmp_buf time_out_clique;
using namespace std;
Timer timeOfAll_clique;
bool TimeOut_clique=false;
ept tl_clique;
ui lb_clique=0;
bool lbTooBig;
GraphClique::GraphClique(const char *_dir) {
	dir = string(_dir);
...
inline  std::string integer_to_string(long long number) {
...
inline static int fileSuffixPos(char* filepath) {
	int j = strlen(filepath) - 1;
	while (filepath[j] != '.')
		j--;
	return j + 1;
}
int GraphClique::main_maximum_clique(char *file_path,int lower_bound,int binary_flag,double time_limit,char * type){
...
#endif
	printf(" ***\n");
	tl_clique=time_limit*1000000;
	GraphClique *graph = new GraphClique(file_path);
...
void GraphClique::read_graph_binary() {
#ifndef NDEBUG
	...
	for(ui i = 0;i < n;i ++) {
		if(degree[i] > 0) {
			...
			// remove self loops and parallelf edges
			ui *buff = edges+pstart[i];
			sort(buff, buff+degree[i]);
			ui idx = 0;
			for(ui j = 0;j < degree[i];j ++) {
...
void GraphClique::read_graph(const char *input_file) {
...
void GraphClique::heuristic_maximal_clique(char opt) {
	Timer t;
	...
	if(UB<lb_clique)
	if(!lbTooBig){
	printf("There are not any clique not smaller than the lower bound\n");
	lbTooBig=true;
	longjmp(time_out_clique,1);
	}
	delete[] color;
	delete[] core;
	delete[] peel_sequence;
	if(max_clique.size()>=lb_clique)
		if(max_clique.size() < UB) printf("\tHeuristic Clique Size: %lu, UB: %u, Running Time: %s (microseconds)\n", max_clique.size(), UB, Utility::integer_to_string(t.elapsed()).c_str());
		else printf("\tMax Clique Size: %lu, Running Time: %s (microseconds)\n", max_clique.size(), Utility::integer_to_string(t.elapsed()).c_str());
	else 
	printf("Heuristic Clique Size is smaller than lowerbound\n");
}

// exact = 1: compute exact maximum clique
// initial_by_MC_EGO = 1: initialize by MC_EDGE
void GraphClique::maximum_clique_color(char exact, char initial_by_MC_EGO) {
	timeOfAll_clique.restart();
	ui *peel_sequence = new ui[n];
	ui *core = new ui[n];
	ui *color = new ui[n];
	...
	if(timeOfAll_clique.elapsed()>=tl_clique){
		TimeOut_clique=true;
		longjmp(time_out_clique,1);
	}
	...
	if(timeOfAll_clique.elapsed()>=tl_clique){
		...
		if(UB<lb_clique)
			...
			printf("\tMC-EGO Time: %s\n", Utility::integer_to_string(timeOfAll_clique.elapsed()).c_str());
		if(timeOfAll_clique.elapsed()>=tl_clique){
		TimeOut_clique=true;
		longjmp(time_out_clique,1);
	}
	...
	delete[] peel_sequence;
	if(max_clique.size()>=lb_clique)
		if(exact||UB <= max_clique.size()) printf("\tMaximum Clique Size: %lu, Max Depth: %u, Running Time: %s (microseconds)\n", max_clique.size(), max_depth, Utility::integer_to_string(timeOfAll_clique.elapsed()).c_str());
		else printf("\tHeuristic Clique Size: %lu, UB: %u, Total Time: %s (microseconds)\n", max_clique.size(), UB, Utility::integer_to_string(timeOfAll_clique.elapsed()).c_str());
	else
	printf("Maximum Clique Size is smaller than lowerbound\n");
}

void GraphClique::output_clique() {
	if(max_clique.size()<lb_clique) return;
	if(TimeOut_clique){
	printf("Maximum Clique Size(may not optimal): %lu,Max Depth: %u, Running Time: %s (microseconds)\n", max_clique.size(), max_depth,Utility::integer_to_string(timeOfAll_clique.elapsed()).c_str());              		
	}
	FILE *fout = Utility::open_file("output/clique.txt", "w");
	fprintf(fout, "%lu\n", max_clique.size());
...
void GraphClique::verify_clique() {
	vector<ui> clique;
...
ui GraphClique::coloring_adj_list(const ui *vs, const ui vs_size, const ui original_size, ui *color, char *vis, const ui start_idx, const ui start_color) {
...
ui GraphClique::degeneracy_ordering_and_coloring_adj(ui *vs, ui *color, const ui vs_size, char *vis, ui *degree, ui *label, const ui K, const ui threshold) {
...
ui GraphClique::degeneracy_maximal_clique_adjacency_list(ui *peel_sequence, ui *core, ui *color, char opt, char greedy_extend) {
	...
	if(new_size == 0) UB = max_clique.size();
		...
		LinearHeap *heap = new LinearHeap(n, new_size-1);
		...
		printf("*** Degeneracy clique size is smaller than lower bound, Time: %s (microseconds)\n",Utility::integer_to_string(t.elapsed()).c_str());
...
ui GraphClique::degeneracy_maximal_clique_adj_list(const ui current_clique_size, ui *vs, const ui vs_size, char *vis, ui *degree, LinearHeap *heap) {
	assert(vs_size > 0);
...
ui GraphClique::ego_degen(const ui *peel_sequence, const ui *core, const ui *color, ui *local_UBs, const ui UB) {
#ifndef NDEBUG
	...
	LinearHeap *heap = new LinearHeap(n, max_core);
...
void GraphClique::heuristic_max_clique_max_degree(ui processed_threshold) {
	Timer t;
	...
	printf("*** Heuristic clique size is smaller than lower bound, time: %s (microseconds)\n",Utility::integer_to_string(t.elapsed()).c_str());
}

//Build the oriented graph
void GraphClique::shrink_graph(ui *&peel_sequence, ui *&core, ui *&color, ui *&out_mapping, ept *&pstart_o, ui *&edges_o) {
	ui *rid = new ui[n];
...
void GraphClique::recursive_search_clique_color_with_kernelization(const ui level, ui clique_size, ui vs_begin, ui vs_size, char kernel) {
...
void GraphClique::recursive_search_clique_color_without_kernelization(const ui level, ui clique_size, ui vs_begin, ui vs_size) {
	assert(clique_size <= max_clique.size()&&vs_buf.size() == color_buf.size()&&vs_size);
	ui *vs = vs_buf.data() + vs_begin;
	ui *color = color_buf.data() + vs_begin;
...
void GraphClique::search_oriented(const ui *peel_sequence, const ui *core, const ui *color, const ui *local_UBs) {
#ifndef NDEBUG
	for(ui i = 0;i < n;i ++) assert(!vis[i]);
	assert(max_clique.size() >= 2&&max_clique.size() <= n);
#endif
